		...
		validateAll(req.query, validation.generateReport_rules, messages)
			.then((data) =>{
				MySQL.pool.getConnection((err,connection)=>{
					...
					connection.query("SELECT name,'Inc' as type, amount, Null as amount_paid, month, notes FROM incomes LEFT JOIN months_has_incomes ON incomes.id = months_has_incomes.incomes_id LEFT JOIN months ON months_has_incomes.months_id = months.id WHERE users_id = ? AND months.id = ? AND incomes.year = ? Union SELECT name,'Exp' as type, amount_due as amount, amount_paid, month, notes FROM expenses LEFT JOIN months_has_expenses ON expenses.id = months_has_expenses.expenses_id LEFT JOIN months ON months_has_expenses.months_id = months.id WHERE users_id = ? AND months.id = ? AND expenses.year = ?",[data.user_id,data.month,data.year,data.user_id,data.month,data.year],(err,rows)=>{
							connection.release();
							if(!err) {
						  	res.json(rows);
							}else {
